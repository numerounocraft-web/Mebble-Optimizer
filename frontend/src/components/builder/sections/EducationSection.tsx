...
export default function EducationSection({ entries, onChange, hideAddButton }: Omit<Props, "darkMode">) {
  ...
  if (entries.length === 0) {
    return <p style={{ fontSize: "12px", color: "#C3C3C3", fontFamily: FONT, letterSpacing: "-0.02em", margin: 0 }}>No education entries yet. Use &ldquo;Add Education&rdquo; above.</p>;
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
      {entries.map((entry, idx) => (
        <div key={entry.id} style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: "11px", fontWeight: 600, color: "#ABABAB", letterSpacing: "-0.02em", fontFamily: FONT }}>
              Education {idx + 1}
            </span>
      ...
      {!hideAddButton && (
        <button onClick={() => onChange([...entries, { id: crypto.randomUUID(), institution: "", degree: "", field: "", startDate: "", endDate: "", gpa: "" }])}
          style={{ display: "flex", alignItems: "center", gap: "5px", padding: "7px 12px", borderRadius: "9999px", border: `1px solid ${BORDER}`, backgroundColor: "#fff", cursor: "pointer", fontSize: "11px", fontWeight: 600, color: "#020202", letterSpacing: "-0.02em", fontFamily: FONT, alignSelf: "flex-start" }}>
