...
function sortByDate(ents: ExperienceEntry[], dir: "asc" | "desc"): ExperienceEntry[] {
  return [...ents].sort((a, b) => {
    const da = parseJobDate(a.startDate);
    const db = parseJobDate(b.startDate);
...
export default function ExperienceSection({ entries, onChange, hideAddButton }: Props) {
  ...
  // Keep openId valid when entries change
  useEffect(() => {
    setOpenId((prev) => {
      if (prev && entries.find((e) => e.id === prev)) return prev;
  ...
  function applySort(dir: "asc" | "desc") {
    onChange(sortByDate(entries, dir));
  }
  ...
  if (entries.length === 0) {
    return (
      <p style={{ fontSize: "12px", color: "#C3C3C3", fontFamily: FONT, letterSpacing: "-0.02em", margin: 0 }}>
  ...
  function renderEntry(entry: ExperienceEntry, idx: number) {
    ...
    return (
      <div
        key={entry.id}
        style={{ backgroundColor: "#FAFAFA", borderRadius: "12px", display: "flex", flexDirection: "column", flexShrink: 0 }}
      ...
      >
        ...
        >
          <span style={{ flex: 1, fontSize: "12px", fontWeight: 600, color: "#8A8A8A", letterSpacing: "-0.02em", fontFamily: FONT, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {label}
          </span>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "24px", padding: "16px 12px 12px" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                ...
                <label style={{ display: "flex", alignItems: "center", gap: "6px", cursor: "pointer" }}>
                  <div
                    onClick={() => updateEntry(entry.id, { current: !entry.current, endDate: "" })}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: "6px", paddingBottom: "4px" }}>
        ...
        <span style={{ color: "#D0D0D0", fontSize: "11px" }}>·</span>
        <button
      ...
      {!hideAddButton && (
        <button
          onClick={() => onChange([...entries, { id: crypto.randomUUID(), company: "", title: "", location: "", startDate: "", endDate: "", current: false, bullets: [""] }])}
          style={{ display: "flex", alignItems: "center", gap: "5px", padding: "7px 12px", borderRadius: "9999px", border: "1px solid #F0F0F0", backgroundColor: "#fff", cursor: "pointer", fontSize: "11px", fontWeight: 600, color: "#020202", letterSpacing: "-0.02em", fontFamily: FONT, alignSelf: "flex-start", marginTop: "4px" }}
        >
